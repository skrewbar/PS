...
struct Line {
    ...
    Fraction getIntersection(const Line& l) {
        return Fraction(slope - l.slope, l.yinter - yinter);
    }
};
...
struct LineContainer {
    ...
    Fraction query(const Fraction& x) {
        const Line& l0 = *(upper_bound(lines.begin(), lines.end(), x,
        ...
        for (const Line& l : buffer)
            ret = max(ret, x * l.slope + l.yinter);
...
int main() {
    LineContainer con(10);
    con.push(Line(1, 10));
    cout << (ll)con.query(2).numer;
}